...
GIT_TAG = "${@os.popen('git ls-remote --tags -q --sort=-version:refname git://github.com/rdkcentral/IPOEHealthCheck.git ' + ' v' + d.getVar('TAG_VERSION', True) + ' RC' + d.getVar('TAG_VERSION', True) + '[a-z]').read().strip().split()[1].split('/')[-1]}"
